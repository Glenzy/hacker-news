...
describe('<Main /> component', () => {
  ...
  const MOCK_ERROR = 'WHOOPS! It broke, 1 moment while we wake our developer up ...';

  it('should mount', async () => {
    const { getByText } = render(<Main stories={MOCK} />);
    expect(getByText('Disabling Snaps in Ubuntu 20.04')).toBeInTheDocument();
  });

  it('should show an error message', async () => {
    const { getByText } = render(<Main error={MOCK_ERROR} />);
    expect(
      getByText('WHOOPS! It broke, 1 moment while we wake our developer up ...')
    ).toBeInTheDocument();
  });
});
